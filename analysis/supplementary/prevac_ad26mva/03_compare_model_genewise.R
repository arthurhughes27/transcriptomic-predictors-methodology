# analysis/supplementary/prevac_ad26mva/03_compare_model_genewise.R
#
# SUPPLEMENTARY comparison: predictive-model choices for predicting PREVAC
# Ad26/MVA (+ placebo) day-180 antibody titer from day-7 gene expression,
...
res <- run_or_load_comparison(
  dataset = "prevac_ad26mva", label = "model_genewise",
...
save_comparison_metrics(res, dataset = "prevac_ad26mva", category = "model_genewise", subdir = "supplementary")

p <- plot(res, metric = "R2") +
  ggtitle("Pipeline comparison: gene-wise model choice (PREVAC Ad26/MVA)")

print(p)

save_pipeline_comparison_plot(p, figure_path, "comparison_model_genewise_prevac_ad26mva.pdf")
