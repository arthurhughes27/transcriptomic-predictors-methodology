...
arm_group = c("SDY1276-Influenza (IN)")
...
# Define alternative pipelines for selection
...
option_choices = list(
  ...
  "Relative gain (threshold = 0)" = list(method = "relative_gain", threshold = 0, relative_gain_inner_folds = 5, relative_gain_metric = "rmse", relative_gain_seed = 12345),
  "Relative gain (threshold = 0.1)" = list(method = "relative_gain", threshold = 0.1, relative_gain_inner_folds = 5, relative_gain_metric = "rmse", relative_gain_seed = 12345),
...
res= predictomics::predict_cv(X = X,
                              Y = Y,
                              engineering_params = list(
                                method = "engineer",
                                col_transform = "z",
                              ...
                              selection_params = list(method = "dearseq", threshold = 0.05, dearseq_mode = "paired", dearseq_level = "geneset", dearseq_covariates = covariates[,-3], genesets = genesets, dearseq_which_test = "asymptotic", dearseq_preprocessed = TRUE, dearseq_n_perm = 5000),
                              model_params     = list(method = "glmnet", inner_folds = 5, metric = "r2"),
                              treatment = NULL,
...
p1 = p1 + ggtitle(label = "Pipeline comparison: feature selection (SDY1276-TIV)")

p1

ggsave(fs::path(figure_path, "Figure6-6.pdf"),
...
p1 = p1 + ggtitle(label = "Pipeline comparison: feature selection (PREVAC-Ad26/MVA)")

p1

ggsave(fs::path(figure_path, "Figure6-7.pdf"),
       p1, width = 9, height = 4.5, dpi = 300)
