...
g_title <- arrangeGrob(
  g,
  top = textGrob(
    "CV prediction of TIV vaccine antibody response in all participants (reference model)",
...
ggsave(fs::path(figure_path, "Figure6-1.pdf"),
...
g_title <- arrangeGrob(
  g,
  top = textGrob(
    "CV prediction of TIV vaccine antibody response in females (reference model)",
    gp = gpar(fontsize = 22, fontface = "bold")
  )
)
...
ggsave(fs::path(figure_path, "Figure6-2.pdf"),
       g_title, width = 15, height = 4.8, dpi = 300)
...
g_title <- arrangeGrob(
  g,
  top = textGrob(
    "CV prediction of TIV vaccine antibody response in Males (reference model)",
    gp = gpar(fontsize = 22, fontface = "bold")
  )
)
...
ggsave(fs::path(figure_path, "Figure6-3.pdf"),
       g_title, width = 15, height = 4.8, dpi = 300)
