...
is2_expr = all_norm_eset@assayData[["exprs"]] %>% 
  t() %>% 
  as.data.frame()
...
sample_info_all_norm = rownames(is2_expr)
...
p_save_all_norm <- fs::path(processed_data_folder, "is2_expr.rds")
...
# Save dataframe
saveRDS(is2_expr, file = p_save_all_norm)
