...
saveRDS(best, file = fs::path(derived_data_dir, "best_pipeline_prevac_rvsv.rds"))
...
saveRDS(best_fit, file = fs::path(derived_data_dir, "best_model_fit_prevac_rvsv.rds"))
...
if (!is.null(p_fit)) {
  print(p_fit)
  ggsave(fs::path(figure_path, "best_model_fit_prevac_rvsv.pdf"), p_fit, width = 6, height = 4.5, dpi = 300)
}
...
if (!is.null(best$selection_params)) {
  ...
  if (!is.null(p_stability)) {
    print(p_stability)
    ggsave(
      fs::path(figure_path, "best_model_selection_stability_prevac_rvsv.pdf"),
