# analysis/pipeline_comparisons/prevac_rvsv/01_prepare_data.R
#
# Build and cache the analysis-ready dataset used by the pipeline-comparison
# scripts in this folder (02-04).
#
# Dataset: PREVAC rVSV vs. placebo, predicting the anti-Ebola-glycoprotein
...
# indicator (1 = rVSV, 0 = placebo) is built for use as the contrast in
...
prevac_rvsv_single <- build_prediction_dataset(
...
prevac_rvsv_paired <- build_paired_dataset(
  df_merged = df_merged_all,
  study_vaccine_groups = study_vaccine_groups,
...
cat(sprintf(
  "PREVAC rVSV (+ placebo): %d participants (%d rVSV, %d placebo), %d gene features (single); %d participants (paired timepoints).\n",
  length(prevac_rvsv_single$participant_id),
  sum(prevac_rvsv_single$treatment == 1),
  sum(prevac_rvsv_single$treatment == 0),
  length(prevac_rvsv_single$gene_names),
  length(unique(prevac_rvsv_paired$participant_id))
))

saveRDS(
  list(
    single = prevac_rvsv_single,
    paired = prevac_rvsv_paired,
    genesets = genesets,
    covariate_names = default_covariates
  ),
